...
if [ -z "${LE_SERVER}" ]; then
	export LE_SERVER="https://acme-v02.api.letsencrypt.org/directory"
fi
...
if [ -z "${PCF_OPSMGR}" ]; then
	echo NO PCF_OPSGR set
	exit 1
fi
if [ -z "${PCF_PASSWD}" ]; then
	echo Please enter your PCF Ops Manager password
	exit 1
fi
if [ -z "${GCP_CERT_NAME}" ]; then
	GCP_CERT_NAME=pcf-cert-$(uuid)
	echo Setting GCP_CERT_NAME to "${GCP_CERT_NAME}"
fi
if [ -z "${GCP_HTTPS_PROXY}" ]; then
	echo No GCP_HTTPS_PROXY
	exit 1
fi
if [ -z "${OPSMAN_CERT_NAME}" ]; then
	echo Setting OPSMAN_CERT_NAME to Certificate
	OPSMAN_CERT_NAME="Certificate"
fi
if [ -z "${GCP_DNS_WAIT}" ]; then
	echo Setting DNS Propogation wait timer to 120
	GCP_DNS_WAIT=120
fi
if [ -z "${SKIP_PAS_CERT}" ]; then
	echo Updating PAS Certificate by default
	SKIP_PAS_CERT=false
fi
if [ -z "${SKIP_PKS_CERT}" ]; then
	echo Updating PKS Certificate by default
	SKIP_PKS_CERT=false
fi
if [ -z "${SKIP_HARBOR_CERT}" ]; then
	echo Updating Harbor Certificate by default
	SKIP_HARBOR_CERT=false
fi
if [ -z "${SKIP_RABBIT_MQ_CERT}" ]; then
	echo Updating RabbitMQ Certificate by default
	SKIP_RABBIT_MQ_CERT=false
fi
if [ -z "${SKIP_GCP_CERT}" ]; then
	echo Updating GCP Certificate by default
	SKIP_GCP_CERT=false
fi
if [ -z "${SKIP_OPSMAN_APPLY}" ]; then
	echo Applying changes in ops manager by default
